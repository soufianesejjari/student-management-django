...
def build_student_schedule_pdf_data(student, start_date, end_date, academic_year=None):
    ...
    enrollments = Enrollment.objects.filter(
        ...
        'assigned_sessions__course',
        'assigned_sessions__room',
        'assigned_sessions__teacher__user',
    )
    ...
    for enrollment in enrollments:
        ...
        assigned_sessions = sorted(
            enrollment.assigned_sessions.all(),
            key=lambda session: (session.day_of_week, session.start_time, session.start_date, session.id),
        )
        courses_dict[key] = {
            'course_name': enrollment.course.name,
            'teacher_name': '',
            'needs_schedule_assignment': not assigned_sessions,
            'sessions': [],
        }
        teacher_names = []
        ...
        courses_dict[key]['teacher_name'] = ' · '.join(dict.fromkeys(teacher_names))
